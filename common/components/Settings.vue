<template>
    <BasePage :title="$t('settings')">
        <ScrollView>
            <StackLayout>
                <SettingSwitchWithSubtitle v-if="biometricsAvailable" :title="$tc('biometric_lock')" :subtitle="$tc('biometric_lock_desc')" v-model="biometricsEnabled" />
                ...
                <SettingSwitchWithSubtitle :title="$tc('send_crash_reports')" :subtitleMaxLines="0" :subtitle="$tc('send_crash_reports_desc')" v-model="sendCrashReports" />
                <StackLayout v-if="userSettings">
                    ...
                    <!-- <GridLayout columns="auto,*,auto" height="80" padding="5">
                        ...
                        <Slider col="1" maxValue="200" :value="newproNotifSettings.radius" @valueChange="newproNotifSettings.radius = Math.floor(parseFloat($event.value))" />
                        <Label verticalTextAlignment="center" col="2" fontSize="14" color="#676767" :text="newproNotifSettings.radius + ' km'" />
                    </GridLayout> -->
