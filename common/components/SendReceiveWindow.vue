                ...
                <PagerItem>
                    <GridLayout rows="auto,auto,*">
                        ...
                        <PullToRefresh ref="pullRefresh" @refresh="refreshHistory" row="2">
                            <CollectionView :items="transactions" :itemIdGenerator="(item, i) => i" rowHeight="80">
                                <v-template>
                                    <GridLayout columns="50,*" padding="5 25 4 0">
                                        <StackLayout col="1" verticalAlignment="center">
                                            <Label :fontSize="17" :text="item.description" textWrap="true">
                                                <Span :text="item.amount.split('.')[0]" />
                                                <Span text="," fontSize="14" />
                                                <Span :text="item.amount.split('.')[1]" fontSize="14" />
                                                <Span :text="' ' + symbol" />
                                            </Label>
                                            <Label :fontSize="14" :text="item.relatedUser && item.relatedUser.display" :color="subtitleColor" paddingTpo="2" />
                                        </StackLayout>
